'''
    A deep learning model for semantic segmentation using EfficientNetB0 as a backbone and integrating a
    Dilated Spatial Pyramid Pooling (DSPP) module for multiscale feature extraction.
    EfficientNet -> Perform scaling of the NN based on Depth, Width, Resolution
    Dilated Convolution -> Same as convolution but it involves pixel skipping,so as to cover a larger area of the input.
...
# This function creates a Dilated Spatial Pyramid Pooling (DSPP) module,
# which captures multiscale information by applying convolutions with different dilation rates
def DilatedSpatialPyramidPooling(dspp_input):
    dims = dspp_input.shape
    # Performs global average pooling on the input feature map, reducing it to a single value for each channel
...
def getModel():
    ...
    # An EfficientNetB0 model pre-trained on ImageNet is used as the backbone
